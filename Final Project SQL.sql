...
-- 4. Courses with more than 5 students (FIXED: Added group by)
...
-- 13. Extract year
select FirstName, LastName, year(EnrollmentDate) as EnrollYear from Students;

-- 14. Full name of instructor
select concat(FirstName, ' ', LastName) as InstructorName from Instructors;

-- 15. Running total (FIXED: Unified count element)
select EnrollmentID, StudentID, CourseID, EnrollmentDate, 
       count(EnrollmentID) over (order by EnrollmentDate) as RunningTotal 
from Enrollments;

-- 16. Senior or Junior (FIXED: Adjusted threshold calculation logic for standard 4-year track)
select FirstName, LastName, EnrollmentDate, 
       case 
            when year(curdate()) - year(EnrollmentDate) >= 3 then 'Senior' 
            else 'Junior' 
